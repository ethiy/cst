        ...
        \subsection{Results}
            \begin{frame}{Used Data}
                \begin{itemize}[leftmargin=6em, font=\color{IGNDarkGreen}]
                    ...
                    \item[Algorithm:]<5-> $\text{Bati3D}^\text{\textregistered}$~\cite{Durupt2006}.
                \end{itemize}
            \end{frame}
            ...
            \begin{frame}{Discussion}
                \begin{itemize}[label=$\blacktriangleright$, font=\color{IGNGreen}]
                    \item Unbalanced \textbf{Building Errors} ($24\%$ among the $502$ instances) $\Rightarrow$ small recall.
                    \item Among the \textbf{Facet Errors}, all but \textbf{Facet Over segmentation} suffer from the unbalanced dataset (ratio $ < 7\%$) $\Rightarrow$ small recall and big overall accuracy.
                    \item Geometric features alone can predict errors with the same order of accuracy.
                \end{itemize}
            \end{frame}
